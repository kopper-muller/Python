...
from database_operacoes import alterar_registro_tabela_categorias, alterar_registro_tabela_contatos, alterar_registro_tabela_marcas, alterar_registro_tabela_produtos, apagar_registro_tabela_categorias, apagar_registro_tabela_contatos, apagar_registro_tabela_marcas, apagar_registro_tabela_produtos, conectar, consultar_registros_tabela_categorias, consultar_registros_tabela_clientes, consultar_registros_tabela_marcas, consultar_registros_tabela_produtos, consultar_tabela_contatos, definir_banco_dados, inserir_registro_tabela_categorias, inserir_registro_tabela_clientes, inserir_registro_tabela_contatos, inserir_registro_tabela_marcas, inserir_registro_tabela_produtos, setup
...
def menu():
    ...
    opcoes = [
        ...
        questionary.Choice("Contatos de clientes", 5),
...
def editar_produto():
    ...
    produto_escolhido["nome"]=questionary.text(
        "Digite o nome do produto: ", default=produto_escolhido.get("nome", ""),).ask().strip()
    produto_escolhido["categoria"]=questionary.select(
        "Escolha a categoria: ", categorias_choices, default= produto_escolhido.get("categoria"),).ask()
...
def apagar_produto():
    ...
    if confirmacao == False:
        return
    apagar_registro_tabela_produtos(produto_escolhido.get("id", 0))
...
def listar_todos_contatos():
    ...
    tabela = Table()
    tabela.add_column("Código")
    tabela.add_column("Nome do cliente")
...
def cadastrar_contato():
    tipo = questionary.text("Digite a forma de contato: ").ask().strip()
    
    valor = questionary.text("Preencha o contato: ").ask().strip()

    id_cliente = questionary.text("Digite o ID do cliente referente à este contato:").ask().strip()
    inserir_registro_tabela_contatos(id_cliente, tipo, valor )


def editar_contato():
    # Consultar as contatos do banco de dados
    contatos = consultar_tabela_contatos()
    # Verificando se não existe contatos cadastradas
    if len(contatos) == 0:
        print("Nenhum contato cadastrado, não sendo possível editar neste momento.")
        # Encerra a execução da função editar, ou seja, não irá apresentar a opção para escolher
        return
    # Criar uma lista de Choice para o usuário poder escolher utilizando o questionary
    contatos_choices = []
    for contato in contatos:
        contatos_choices.append(questionary.Choice(contato.get("cliente", {}).get("nome"), contato))
    # Perguntar para o usuário qual a contato que ele deseja editar
    contato_escolhido = questionary.select("Escolha o contato para editar:", contatos_choices).ask()
    print(contato_escolhido)
    print(contato_escolhido)

    contato_escolhido["cliente"]["id"] = questionary.text(
        "Digite o id do cliente dono deste contato: ", str(contato_escolhido.get("cliente", {}).get("id")), 
    ).ask().strip()

    contato_escolhido["valor"] = questionary.text(
        "Digite o 'valor' do contato: ",  contato_escolhido.get("valor"),
    ).ask().strip()

    contato_escolhido["tipo"] = questionary.text(
        "Digite o tipo do contato: ", contato_escolhido.get("tipo")
    ).ask().strip()
        
    # Chamar a função que executará o update na tabela de categoiras, 
    # para efetivar a atualização na base de dados
    alterar_registro_tabela_contatos(
        contato_escolhido.get("id_cliente"), 
        contato_escolhido.get("valor"),
        contato_escolhido.get("tipo")
    )
...
def menu_contatos():
    opcao = 0
    opcoes = [    
        questionary.Choice("Listar todos os contatos", 1),
        questionary.Choice("Cadastra contatos", 2),
        questionary.Choice("Editar contatos", 3),
        questionary.Choice("Apagar contatos", 4),
        questionary.Choice("Voltar", 1000),
    ]
    while opcao != 1000:
...
def apagar_contatos():
    ...
    # Verificando se não existe marcas cadastradas
    if len(contatos) == 0:
        print("Nenhum contato cadastrado, não sendo possível apagar neste momento.")
        # Encerra a execução da função apagar, ou seja, não irá apresentar a opção para escolher
        return 
    # Criar uma lista de Choice para o usuário poder escolher utilizando o questionary
    contatos_choices = []
    for contato in contatos:
        contatos_choices.append(questionary.Choice(contato.get("cliente", {}).get("nome") +"(" + contato.get("tipo")+")", contato))
    # Perguntar para o usuário qual o contato que ele deseja apagar
    contato_escolhido = questionary.select("Escolha o contato para apagar:", contatos_choices).ask()
    # Executar o delete na tabela de contatos com o id do contato escolhido
    texto = f"Deseja realmente apagar o registro {contato_escolhido.get('nome')}"
    confirmacao = questionary.confirm(texto, default=False).ask()
    if confirmacao == False:
        return
